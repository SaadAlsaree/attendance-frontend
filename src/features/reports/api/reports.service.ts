...
import { GetAttendanceReportQuery, AttendanceReportResponse } from '../types/attendance-reports';
import { OrganizationalSummaryQuery, OrganizationalSummaryResponse } from '../types/organizational-summary';
...
export const reportsService = {
    ...
    async getAttendanceSummary(query?: GetAttendanceReportQuery): Promise<AttendanceReportResponse | null> {
        try {
            const response = await axiosInstance.get(`${baseUrl}/reports/attendance-summary`, { params: query });
    ...
    async getAttendanceSummaryClient(query: GetAttendanceReportQuery): Promise<AttendanceReportResponse | null> {
        try {
            const response = await axiosClient.get(`${baseUrl}/reports/attendance-summary`, { params: query });
            if (response.status >= 400) {
                console.error('Error fetching attendance report:', response.statusText);
                return null;
            }
    ...
    async getOrganizationalSummaryClient(query: OrganizationalSummaryQuery): Promise<OrganizationalSummaryResponse | null> {
        try {
            const response = await axiosClient.get(`${baseUrl}/reports/organizational-summary`, { params: query });
            if (response.status >= 400) {
                console.error('Error fetching comprehensive attendance report:', response.statusText);
                return null;
            }
    ...
    async getOrganizationClient(query: OrganizationalReportRequest): Promise<OrganizationalSummaryResponse | null> {
        try {
            const response = await axiosClient.get(`${baseUrl}/reports/organization`, { params: query });
            if (response.status >= 400) {
                console.error('Error fetching organization:', response.statusText);
                return null;
            }
