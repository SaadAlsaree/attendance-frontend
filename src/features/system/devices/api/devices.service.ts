...
export const devicesService = {
  ...
  async getDevicesList(query: DeviceQuery): Promise<DeviceResponse | null> {
    try {
      const response = await axiosInstance.get(`${baseUrl}/devices`, {
        params: query
      });
  ...
  async getDevicesListClient(
    query: DeviceQuery
  ): Promise<DeviceResponse | null> {
    try {
      const response = await axiosClient.get(`${baseUrl}/devices`, {
        params: query
      });
      if (response.status >= 400) {
        console.error('Error fetching devices list:', response.statusText);
