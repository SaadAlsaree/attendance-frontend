...
export const shiftService = {
  ...
  async getShiftsListClient(
    query: ShiftQuery
  ): Promise<ShiftListResponse | null> {
    try {
      const response = await axiosClient.get(`${baseUrl}/shifts`, {
        params: query
      });
      if (response.status >= 400) {
        console.error('Error fetching shifts list:', response.statusText);
